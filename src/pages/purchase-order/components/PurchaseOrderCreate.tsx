...
const PurchaseOrderCreatePage = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const [supRes, prodRes] = await Promise.all([
        SupplierService.getSupplierList({ page: 1, limit: 100, criteria: {}, sort_bys: [] }),
        ProductService.getProductList({ page: 1, limit: 100, criteria: {}, sort_bys: [] }),
      ]);
  ...
  const handleAddItem = () => {
    form.insertListItem("items", { product_id: "", quantity: 1, unit_price: 0 });
  };
  ...
  const calculateTotal = () => {
    return form.values.items.reduce((sum, item) => sum + (item.quantity * item.unit_price), 0);
  };
    ...
    <Table.Tr key={index}>
      <Table.Td>
        <Select
          ...
          data={products.map((p) => ({ value: p.product_id, label: `${p.sku} - ${p.name}` }))}
      ...
      <Stack gap="lg">
        <Group justify="space-between">
          <Group gap="sm">
            ...
            <Stack gap={0}>
              <Title order={2} fw={700}>Create Purchase Order</Title>
              <Text c="gray.6" fz="sm">Fill in the details to create a new purchase order.</Text>
            </Stack>
          </Group>
          <Button 
            ...
            <Paper withBorder p="md" radius="md">
              <Group grow align="flex-start">
                <Select
                  ...
                  data={suppliers.map((s) => ({ value: s.supplier_id, label: s.name }))}
